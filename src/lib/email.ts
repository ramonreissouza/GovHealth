...
// não estiver configurada, apenas loga e segue (não quebra o webhook).
...
export async function enviarBoasVindas(params: {
      ...
      <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:32px 16px;">
        <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;overflow:hidden;">
          ...
          <tr><td style="padding:26px;">
            <h1 style="font-size:19px;color:#0f172a;margin:0 0 6px;">Assinatura confirmada 🎉</h1>
            <p style="font-size:13px;color:#334155;margin:0 0 14px;">
              ${params.nome ? params.nome + ', ' : ''}sua assinatura do plano <strong>${params.plano}</strong> está ativa.
            </p>
            ${bloco}
            <a href="${login}" style="display:inline-block;margin-top:12px;background:#059669;color:#fff;font-size:14px;font-weight:600;text-decoration:none;padding:11px 20px;border-radius:9px;">Acessar a plataforma</a>
          </td></tr>
          <tr><td style="padding:14px 26px;background:#f8fafc;border-top:1px solid #eef2f7;font-size:11px;color:#94a3b8;">
            Emitimos nota fiscal. Dúvidas? Responda este e-mail.
          </td></tr>
        </table>
      </td></tr></table>
    </body></html>`
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      from,
      to: params.email,
      subject: `GovHealth.ai — assinatura ${params.plano} ativada`,
      html,
    })
    if (error) { console.warn('[email] falha Resend:', error.message); return { enviado: false, motivo: error.message } }
    return { enviado: true }
  } catch (e) {
    console.warn('[email] erro ao enviar boas-vindas:', e)
