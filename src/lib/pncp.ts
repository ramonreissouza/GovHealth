...
export async function buscarComprasSaude(params: PNCPSearchParams = {}) {
  ...
  // Mesmo orçamento de tempo de buscarLicitacoesAbertas: para de abrir páginas novas
  // quando estoura, para uma função serverless nunca ser morta no meio.
  const inicioMs = Date.now()
  const budgetMs = params.budgetMs ?? Number.POSITIVE_INFINITY
...
// `deadline` (timestamp absoluto) impede o caso que mata função serverless: um retry
// começado no fim do orçamento. Cada tentativa pode levar 25s, então três tentativas
