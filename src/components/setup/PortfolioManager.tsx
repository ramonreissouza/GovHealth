'use client'
// src/components/setup/PortfolioManager.tsx
// Bloco "Meu Portfólio" do Setup da Empresa — cadastro dos produtos que a empresa
// vende (nome, marca/modelo, categoria, palavras-chave, CATMAT). Extraído da antiga
// página /portfolio para ser reutilizado dentro da tela unificada de Setup.

import React, { useState, useEffect, useCallback } from 'react'
import { clsx } from 'clsx'
import {
  Plus, Trash2, Edit2, X, Save, Search, Boxes, Tag, Package,
...
export default function PortfolioManager() {
  ...
  function openNovo() { setEditId(null); setModalOpen(true) }
  function openEdit(id: string) { setEditId(id); setModalOpen(true) }
  function handleDelete(id: string) {
    if (!confirm('Remover este produto do portfólio?')) return
    deleteProduto(id); refresh()
  }
  function handleToggle(id: string) { toggleAtivo(id); refresh() }
      ...
      <div className="flex items-start justify-between mb-5">
        <div className="max-w-[560px]">
          <p className="text-[13px] text-muted">
            ...
            CATMAT e palavras-chave para filtrar as oportunidades — e para alimentar alertas e
            demais telas a partir do seu setup.
          </p>
        </div>
...
function ProdutoModal({
  ...
  const [form, setForm] = useState<ProdutoInput>(
    produto ? { ...EMPTY_FORM, ...produto } : EMPTY_FORM
  )
          ...
          <div className="border-t border-subtle pt-4">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={buscarCatmat} disabled={catmatLoading || !catmatQuery.trim()}
                ...
                {catmatLoading ? <Search size={13} className="animate-pulse" /> : <Search size={13} />}
                Buscar
              </button>
            </div>
