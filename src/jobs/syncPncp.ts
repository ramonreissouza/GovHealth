// src/jobs/syncPncp.ts — SINCRONIZAÇÃO DIÁRIA (agendada via pg-boss, worker; ver
// src/worker/index.ts). A rota src/app/api/cron/sync-pncp/route.ts é hoje só uma
// casca fina para disparo manual/debug via HTTP — quem agenda de verdade é o worker.
//
...
export async function runSyncPncp() {
