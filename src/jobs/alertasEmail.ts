// src/jobs/alertasEmail.ts — resumo DIÁRIO por e-mail dos leads que combinam com os
// monitores do usuário (agendada via pg-boss, worker; ver src/worker/index.ts). A
// rota src/app/api/cron/alertas-email/route.ts é hoje só uma casca fina para disparo
...
export async function runAlertasEmail() {
  // E-mail é best-effort (como todo o app): sem chave, o job não falha — só informa
  // que está inerte até a RESEND_API_KEY ser configurada.
  const apiKey = process.env.RESEND_API_KEY
  if (!apiKey) {
    console.warn('[cron:alertas-email] RESEND_API_KEY não configurada — e-mail inerte')
    return { ok: true as const, skipped: true, motivo: 'RESEND_API_KEY não configurada — e-mail inerte até definir a chave' }
  }
  ...
  if (usuarios.length === 0) {
    return { ok: true as const, usuarios: 0, enviados: 0, documentos: docsAviso, motivo: 'nenhum monitor com e-mail' }
  }
  ...
  console.log(msg)
  return { ok: true as const, usuarios: usuarios.length, enviados, editais: editais.length, emendas: emendas.length, documentos: docsAviso }
}
