...
## 5b. O Norton quebra o tunel dentro do container (13/09/2026)

Sintoma: `cloudflared` sobe e morre com
...
Nao e o Cloudflare nem a rede. O Norton faz "SSL/TLS scanning" — intercepta e
reassina as conexoes. Medido:
...
Do host funciona (o Windows confia nessa raiz). De dentro de um container nao:
ele nao conhece a raiz do Norton e recusa, **corretamente**. Foi isso que
derrubou `winget install cloudflared` com `0x8a15005e`, e nao um defeito do
winget.

Conserto: dar ao container a mesma raiz que a maquina ja confia.

```bash
# 1. exportar a raiz do Norton do armazem do Windows (PowerShell)
#    Get-ChildItem Cert:\LocalMachine\Root | Where Subject -like '*Norton*'
#    e gravar como PEM (Base64)

# 2. juntar com o pacote padrao de raizes
docker run --rm curlimages/curl:latest sh -c 'cat /etc/ssl/certs/ca-certificates.crt' > base.crt
cat base.crt norton-root.pem > ca-bundle.crt

# 3. apontar o Go para o pacote (cloudflared e Go; ele honra SSL_CERT_FILE)
docker run -d --name radar-tunnel --restart no   -v "<caminho>\ca-bundle.crt:/certs/ca-bundle.crt:ro"   -e SSL_CERT_FILE=/certs/ca-bundle.crt   cloudflare/cloudflared:latest tunnel --no-autoupdate --url http://host.docker.internal:3200
```

Isto nao enfraquece nada alem do que ja existe: a maquina inteira ja confia
nessa raiz porque o proprio antivirus a instalou. O container passa a enxergar
o mundo como o host enxerga. A alternativa limpa e excluir
`api.trycloudflare.com` do scanning na interface do Norton.
...
publica-se com `vercel --prod`, que sobe o diretorio de trabalho. O botao
Redeploy reconstroi o ULTIMO UPLOAD — se o codigo novo nunca subiu, ele
reconstroi fielmente o codigo velho. Sintoma exato: a CSP volta **sem
`frame-src` nenhum**, nem mesmo `frame-src 'self'`. Se aparecesse
`frame-src 'self'` sozinho, o codigo estaria la e faltaria so a variavel.

**Variavel existir nao e variavel ter valor.** `RADAR_CONNECT_TOKEN` estava
cadastrada e aparecia na lista do `vercel env ls`, mas com valor `""`. A rota
testa `if (!base || !token)` e string vazia e falsa — o 503 continuaria igual.
Conferir com o valor, nao com a lista:

```bash
vercel env pull /tmp/p.env --environment=production --yes
grep -oE '^RADAR_[A-Z_]+' /tmp/p.env        # os nomes
grep '^RADAR_CONNECT_TOKEN=' /tmp/p.env | wc -c   # o TAMANHO (64 + ruido)
rm -f /tmp/p.env
```
